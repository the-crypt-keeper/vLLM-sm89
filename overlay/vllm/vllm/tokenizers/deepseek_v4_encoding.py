...
# adds nothing.
#
# Verbatim from the official encoder shipped with the checkpoint,
# `<model>/encoding/encoding_dsv4.py:67-80`.  vLLM 0.25.1 shipped a single
# constant named REASONING_EFFORT_MAX that actually held the **high** text, and
# emitted it only for `reasoning_effort == "max"` -- so `high` was silently a
# no-op (identical to `low`), `max` delivered official high, and official `max`
# ("Beyond maximum") was unreachable from any request.  See the reasoning-effort
# section of CLAUDE-TP8.md.
